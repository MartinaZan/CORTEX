            ...
            for ctf_candidate in self.dataset.instances:
                ...
                if ctf_candidate.time <= instance.time and ctf_candidate.time in set(candidates_idx) and \
                    ...
                    if input_label != candidate_label:
                        ...
                        metric = self.metric_coefficient * self.distance_metric.evaluate(instance, ctf_candidate, self.oracle).cpu().numpy()

                        # Time penalty
                        # Questo volendo si potrebbe fare rispetto all'inizio della crisi
                        time_penalty = self.max_time_penalty * ((instance.time - ctf_candidate.time) / instance.time)

                        # Stability penalty
                        stability_penalty = self.stability_penalty * ( values_stability[np.where(candidates_idx == ctf_candidate.time)[0][0]] ** 0.5 )
